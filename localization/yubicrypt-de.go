...
func (g *GUI) createMainUI() fyne.CanvasObject {
	...
	signTextBtn := widget.NewButton("Signieren", g.onSignText)
	padBtn := widget.NewButton("Polstern", g.onPad)
	encryptBtn := widget.NewButton("Verschlüsseln", g.onEncrypt)
	encryptBtn.Importance = widget.HighImportance // Blue
	decryptBtn := widget.NewButton("Entschlüsseln", g.onDecrypt)
	decryptBtn.Importance = widget.HighImportance // Blue
	unpadBtn := widget.NewButton("Entpolstern", g.onUnpad)
	verifyTextBtn := widget.NewButton("Verifizieren", g.onVerifyText)
	...
	clearBtn := widget.NewButton("Löschen", g.onClear)
...
func (g *GUI) onSignText() {
	if g.pinEntry.Text == "" {
		g.statusLabel.SetText("Fehler: PIN zum Signieren erforderlich")
		return
	}

	input := g.textArea.Text
	if input == "" {
		g.statusLabel.SetText("Fehler: Kein Text zum Signieren")
		return
	}
	...
	for algo := range supportedAlgorithms {
		if strings.Contains(s, "-----BEGIN "+algo+" SIGNATURE-----") {
			g.statusLabel.SetText("Fehler: Nachricht enthält bereits eine Signatur")
			return
		}
	}
	...
	if err != nil {
		g.statusLabel.SetText("Signieren fehlgeschlagen: " + err.Error())
		return
	}

	g.textArea.SetText(result)
	g.statusLabel.SetText("✓ Nachricht erfolgreich signiert (" + formatByteSize(len(input)) + ")")
}
...
func (g *GUI) onVerifyText() {
	input := g.textArea.Text
	if input == "" {
		g.statusLabel.SetText("Fehler: Kein Text zum Verifizieren")
		return
	}

	err := g.verifyData([]byte(input))
	if err != nil {
		g.statusLabel.SetText("Verifizierung fehlgeschlagen: " + err.Error())
		return
	}

	g.statusLabel.SetText("✓ Signatur ist gültig")
}

// onEncrypt triggers encryption using a public key
func (g *GUI) onEncrypt() {
	if g.encryptionUsed {
		g.statusLabel.SetText("Bitte wählen Sie ein neues Zertifikat für die Verschlüsselung.")
		g.publicKeyPath = ""
		g.choosePublicKey()
		return
	}
	...
	if g.publicKeyPath != "" {
		input := g.textArea.Text
		if input == "" {
			g.statusLabel.SetText("Fehler: Kein Text zum Verschlüsseln")
			return
		}

		result, err := g.encryptData([]byte(input), g.publicKeyPath)
		if err != nil {
			g.statusLabel.SetText("Verschlüsselung fehlgeschlagen: " + err.Error())
			return
		}

		g.encryptionUsed = true
		g.textArea.SetText(result)
		g.statusLabel.SetText("✓ Verschlüsselt mit: " + filepath.Base(g.publicKeyPath))
		return
	}
...
func (g *GUI) choosePublicKey() {
	dialog.ShowFileOpen(func(reader fyne.URIReadCloser, err error) {
		if err != nil {
			g.statusLabel.SetText("Fehler beim Auswählen der Datei: " + err.Error())
			return
		}
		if reader == nil {
			return
		...
		g.statusLabel.SetText("Ausgewählter öffentlicher Schlüssel: " + filepath.Base(path) + " - Verschlüssle...")

		input := g.textArea.Text
		if input == "" {
			g.statusLabel.SetText("Ausgewählt: " + filepath.Base(path) + " - Kein Text zum Verschlüsseln")
			return
		}

		result, err := g.encryptData([]byte(input), g.publicKeyPath)
		if err != nil {
			g.statusLabel.SetText("Verschlüsselung fehlgeschlagen: %v" + err.Error())
			return
		}

		g.encryptionUsed = true
		g.textArea.SetText(result)
		g.statusLabel.SetText("✓ Verschlüsselt mit: " + filepath.Base(path))
	}, g.window)
}
...
func (g *GUI) onDecrypt() {
	if g.pinEntry.Text == "" {
		g.statusLabel.SetText("Fehler: PIN zum Entschlüsseln erforderlich")
		return
	}

	input := g.textArea.Text
	if input == "" {
		g.statusLabel.SetText("Fehler: Kein Text zum Entschlüsseln")
		return
	}

	result, err := g.decryptData([]byte(input), g.pinEntry.Text)
	if err != nil {
		g.statusLabel.SetText("Entschlüsselung fehlgeschlagen: " + err.Error())
		return
	}

	g.textArea.SetText(string(result))
	g.statusLabel.SetText("✓ Nachricht erfolgreich entschlüsselt")
}
...
func (g *GUI) onClear() {
	...
	g.statusLabel.SetText("Textbereich, Zwischenablage und Verschlüsselungsstatus zurückgesetzt")
}
...
func (g *GUI) signData(data []byte, pin string) (string, error) {
	...
	if len(normalizedData) > 1024*1024 { // > 1MB
		g.statusLabel.SetText("Hashing des großen Dokuments (" + formatByteSize(len(normalizedData)) + ")...")
		g.window.Canvas().Refresh(g.statusLabel)
	}

	sig, algo, err := g.signDataInternal(pinGuard.Bytes(), normalizedData)
	if err != nil {
		return "", fmt.Errorf("signieren fehlgeschlagen: %v", err)
	}
...
func (g *GUI) signDataInternal(pin, data []byte) (string, string, error) {
	...
	if err != nil {
		return "", "", fmt.Errorf("Zertifikat vom Signatur-Slot konnte nicht abgerufen werden: %v", err)
	}
	...
	if !ok {
		return "", "", fmt.Errorf("öffentlicher Schlüssel ist nicht ECDSA oder Ed25519")
	}

	// Algorithm name is "ECCP256", not "P-256"
	algorithm, exists := curveToAlgorithm[pubKey.Curve]
	if !exists {
		return "", "", fmt.Errorf("nicht unterstützte Kurve: %v", pubKey.Curve)
	}
	...
	case crypto.SHA384:
		...
		digest = h.Sum(nil)
	default:
		return "", "", fmt.Errorf("nicht unterstützter Hash-Algorithmus für Kurve")
	}

	auth := piv.KeyAuth{PIN: string(pin)}
	priv, err := yk.PrivateKey(piv.SlotSignature, cert.PublicKey, auth)
	if err != nil {
		return "", "", fmt.Errorf("privater Schlüssel konnte nicht abgerufen werden: %v", err)
	}

	signer, ok := priv.(crypto.Signer)
	if !ok {
		return "", "", fmt.Errorf("Schlüssel implementiert nicht crypto.Signer")
	}
	...
	if _, err := asn1.Unmarshal(asn1sig, &sig); err != nil {
		return "", "", fmt.Errorf("ASN.1 unmarshalling fehlgeschlagen: %v", err)
	}
...
func (g *GUI) signEd25519Data(pin string, hash []byte, pubKey ed25519.PublicKey, yk *piv.YubiKey) (string, string, error) {
	...
	if err != nil {
		return "", "", fmt.Errorf("Ed25519 Signieren fehlgeschlagen: %v", err)
	}
...
func (g *GUI) verifyData(data []byte) error {
	...
	if algorithm == "" {
		g.showErrorPopup("Keine unterstützte Signatur gefunden", []byte{}, "")
		return fmt.Errorf("kein unterstützter Signaturblock gefunden")
	}

	i := strings.Index(s, beg)
	j := strings.Index(s, end)
	if i == -1 || j == -1 || j <= i {
		g.showErrorPopup("Ungültiges Signaturformat", []byte{}, algorithm)
		return fmt.Errorf("ungültiges Signaturblock-Format")
	}
	...
	if err != nil {
		g.showErrorPopup("Hex-Dekodierung fehlgeschlagen", []byte{}, algorithm)
		return fmt.Errorf("Hex-Dekodierung fehlgeschlagen: %v", err)
	}

	// Status for large files
	if len(originalMessage) > 1024*1024 {
		g.statusLabel.SetText("Verifiziere großes Dokument (" + formatByteSize(len(originalMessage)) + ")...")
		g.window.Canvas().Refresh(g.statusLabel)
	}
	...
	default:
		verificationErr = fmt.Errorf("nicht unterstützter Algorithmus: %s", algorithm)
	}

	if verificationErr != nil {
		publicKeyBytes, _ := extractPublicKeyFromSignature(combined, algorithm)
		g.showErrorPopup("Signaturverifizierung fehlgeschlagen: "+verificationErr.Error(), publicKeyBytes, algorithm)
		return verificationErr
	}

	publicKeyBytes, err := extractPublicKeyFromSignature(combined, algorithm)
	if err != nil {
		g.showErrorPopup("Fehler beim Extrahieren des öffentlichen Schlüssels: "+err.Error(), []byte{}, algorithm)
		return err
	}
...
func extractPublicKeyFromSignature(combined []byte, algorithm string) ([]byte, error) {
	...
	case AlgorithmED25519:
		if len(combined) != Ed25519CombinedSize {
			return nil, fmt.Errorf("ungültiger Ed25519 Signaturblock")
		}
	...
	case AlgorithmECCP256, AlgorithmECCP384:
		...
		case AlgorithmECCP384:
			curve = elliptic.P384()
		default:
			return nil, fmt.Errorf("nicht unterstützter ECDSA Algorithmus: %s", algorithm)
		}
		...
		if len(combined) != expectedBytes {
			return nil, fmt.Errorf("ungültige Signaturblockgröße: erwartet %d, erhalten %d", expectedBytes, len(combined))
		}
	...
	default:
		return nil, fmt.Errorf("nicht unterstützter Algorithmus: %s", algorithm)
	}
}
...
func (g *GUI) verifyEd25519(dataHash, combined []byte) error {
	if len(combined) != Ed25519CombinedSize {
		return fmt.Errorf("ungültiger Ed25519 Signaturblock")
	}
	...
	if !ed25519.Verify(ed25519.PublicKey(publicKey), dataHash, signature) {
		return fmt.Errorf("Ed25519 Signaturverifizierung fehlgeschlagen")
	}
...
func (g *GUI) verifyECDSA(data, combined []byte, algorithm string) error {
	...
	default:
		return fmt.Errorf("nicht unterstützter ECDSA Algorithmus: %s", algorithm)
	}
	...
	if len(combined) != expectedBytes {
		return fmt.Errorf("ungültige Signaturblockgröße: erwartet %d, erhalten %d", expectedBytes, len(combined))
	}
	...
	if !curve.IsOnCurve(X, Y) {
		return fmt.Errorf("öffentlicher Schlüsselpunkt (X,Y) liegt nicht auf der Kurve %s", curve.Params().Name)
	}
	...
	if !ecdsa.Verify(pub, digest, R, S) {
		return fmt.Errorf("Signaturverifizierung fehlgeschlagen")
	}
...
func (g *GUI) showSuccessPopup(publicKeyBytes []byte, algorithm string) {
	...
	successLabel := widget.NewLabel("Signatur ist gültig")
	successLabel.Alignment = fyne.TextAlignCenter

	copyBtn := widget.NewButton("Signaturkomponente kopieren", func() {
		clipboard := g.app.Clipboard()
		if clipboard != nil {
			clipboard.SetContent(hexString)
			g.statusLabel.SetText("✓ Signaturkomponente in Zwischenablage kopiert")
			time.AfterFunc(2*time.Second, func() {
				g.statusLabel.SetText("Bereit")
			})
		}
	})
...
func (g *GUI) encryptData(data []byte, pubKeyFile string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("öffentlicher Schlüssel konnte nicht geladen werden: %v", err)
	}
	...
	if _, err := rand.Read(aesKeyGuard.Bytes()); err != nil {
		return "", fmt.Errorf("AES-Schlüsselgenerierung fehlgeschlagen: %v", err)
	}

	encryptedKey, err := rsa.EncryptPKCS1v15(rand.Reader, pubKey, aesKeyGuard.Bytes())
	if err != nil {
		return "", fmt.Errorf("RSA-Verschlüsselung fehlgeschlagen: %v", err)
	}
	defer memguard.WipeBytes(encryptedKey)

	encryptedData, err := encryptAES(data, aesKeyGuard.Bytes())
	if err != nil {
		return "", fmt.Errorf("AES-Verschlüsselung fehlgeschlagen: %v", err)
	}
...
func (g *GUI) decryptData(data []byte, pin string) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("Base64-Dekodierung fehlgeschlagen: %v", err)
	}
	defer memguard.WipeBytes(combined)

	yk, err := openYubiKey(0)
	if err != nil {
		return nil, fmt.Errorf("YubiKey konnte nicht geöffnet werden: %v", err)
	}
	defer yk.Close()

	cert, err := yk.Certificate(piv.SlotKeyManagement)
	if err != nil {
		return nil, fmt.Errorf("Zertifikat von Slot 9d konnte nicht abgerufen werden: %v", err)
	}

	rsaPubKey, ok := cert.PublicKey.(*rsa.PublicKey)
	if !ok {
		return nil, fmt.Errorf("Zertifikat enthält keinen RSA öffentlichen Schlüssel")
	}

	if err := checkRSASecurity(rsaPubKey, "auf YubiKey"); err != nil {
		return nil, err
	}

	keySize := rsaPubKey.Size()
	if len(combined) < keySize {
		return nil, fmt.Errorf("Chiffretext zu kurz")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("privater Schlüssel konnte nicht abgerufen werden: %v", err)
	}

	decrypter, ok := priv.(crypto.Decrypter)
	if !ok {
		return nil, fmt.Errorf("privater Schlüssel unterstützt keine Entschlüsselung")
	}

	decryptedPayload, err := decrypter.Decrypt(rand.Reader, encryptedKey, nil)
	if err != nil {
		return nil, fmt.Errorf("RSA-Entschlüsselung fehlgeschlagen: %v", err)
	}
	defer memguard.WipeBytes(decryptedPayload)

	if len(decryptedPayload) != 32 {
		return nil, fmt.Errorf("ungültige AES-Schlüsselgröße")
	}

	decryptedData, err := decryptAES(encryptedData, decryptedPayload)
	if err != nil {
		return nil, fmt.Errorf("AES-Entschlüsselung fehlgeschlagen: %v", err)
	}
...
func secureUnpadMessage(data []byte) ([]byte, error) {
	if len(data) == 0 {
		return nil, errors.New("Polsterung kann nicht von leeren Daten entfernt werden")
	}
	if len(data)%4096 != 0 {
		return nil, errors.New("ungültige Blockgröße für Polster-Entfernung")
	}
	...
	for i := len(data) - 1; i >= 0; i-- {
		...
		if data[i] != 0x00 {
			return nil, errors.New("ungültiges Polster-Format: unerwartetes Nicht-Null-Byte")
		}
	}

	if lastIndex == -1 {
		return nil, errors.New("kein Polster-Marker gefunden")
	}
...
func (g *GUI) onPad() {
	input := g.textArea.Text
	if input == "" {
		g.statusLabel.SetText("Fehler: Kein Text zum Polstern")
		return
	}
	...
	g.statusLabel.SetText(fmt.Sprintf("✓ Gepolstert: %d -> %d Bytes (Base64)", originalLen, paddedLen))
}

func (g *GUI) onUnpad() {
	input := g.textArea.Text
	if input == "" {
		g.statusLabel.SetText("Fehler: Kein Text zum Entpolstern")
		return
	}

	binaryData, err := base64.StdEncoding.DecodeString(input)
	if err != nil {
		g.statusLabel.SetText("Entpolstern fehlgeschlagen: Ungültige Base64-Daten")
		return
	}

	unpaddedData, err := secureUnpadMessage(binaryData)
	if err != nil {
		g.statusLabel.SetText("Entpolstern fehlgeschlagen: " + err.Error())
		return
	}

	g.textArea.SetText(string(unpaddedData))
	g.statusLabel.SetText("✓ Polsterung erfolgreich entfernt")
}
...
func checkRSASecurity(pubKey *rsa.PublicKey, context string) error {
	...
	if keySize < minRSABits {
		return fmt.Errorf("unsicherer %d-Bit RSA Schlüssel %s - Minimum ist %d-Bit", keySize, context, minRSABits)
	}
	if _, supported := supportedRSASizes[keySize]; !supported {
		fmt.Fprintf(os.Stderr, "WARNUNG: %d-Bit RSA Schlüssel %s - unterstützte Größen sind 2048, 3072, 4096 Bits\n", keySize, context)
	}
	if keySize == 1024 {
		fmt.Fprintf(os.Stderr, "KRITISCHE WARNUNG: 1024-Bit RSA Schlüssel %s sind unsicher und sollten nicht verwendet werden!\n", context)
	}
	return nil
}
...
func loadRSAPublicKey(filename string) (*rsa.PublicKey, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("öffentliche Schlüsseldatei konnte nicht gelesen werden: %v", err)
	}
	...
	case "CERTIFICATE":
		...
		if err != nil {
			return nil, fmt.Errorf("Zertifikat konnte nicht geparst werden: %v", err)
		}
		pubKey, ok := cert.PublicKey.(*rsa.PublicKey)
		if !ok {
			return nil, fmt.Errorf("Zertifikat enthält keinen RSA öffentlichen Schlüssel")
		}
		if err := checkRSASecurity(pubKey, "im Zertifikat "+filename); err != nil {
			return nil, err
		}
		return pubKey, nil
	...
	case "PUBLIC KEY":
		...
		if err != nil {
			return nil, fmt.Errorf("öffentlicher Schlüssel konnte nicht geparst werden: %v", err)
		}
		pubKey, ok := pubInterface.(*rsa.PublicKey)
		if !ok {
			return nil, fmt.Errorf("kein RSA öffentlicher Schlüssel")
		}
		if err := checkRSASecurity(pubKey, "in Datei "+filename); err != nil {
			return nil, err
		}
		return pubKey, nil
	...
	case "RSA PUBLIC KEY":
		...
		if err != nil {
			return nil, fmt.Errorf("RSA öffentlicher Schlüssel konnte nicht geparst werden: %v", err)
		}
		if err := checkRSASecurity(pubKey, "in Datei "+filename); err != nil {
			return nil, err
		}
...
func openYubiKey(index int) (*piv.YubiKey, error) {
	cards, err := piv.Cards()
	if err != nil {
		return nil, fmt.Errorf("Kartenliste konnte nicht abgerufen werden: %v", err)
	}
	if len(cards) == 0 {
		return nil, fmt.Errorf("keine Smartcard gefunden")
	}
	...
	return nil, fmt.Errorf("kein YubiKey am Index %d gefunden", index)
}
...
func (identicon *ClassicIdenticon) Generate() image.Image {
	...
	bgChoice := 0
	for i := 0; i < 2; i++ { // Use only 2 bits as in identicons program
